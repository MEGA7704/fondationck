(()=>{
  ...
  function render(){
    ...
    document.getElementById('reportTable').innerHTML=`<table id="reportPrintable"><thead><tr><th>Secteur</th><th>Localité</th><th>Responsables</th><th>Associations</th><th>Jeunes filles</th><th>Jeunes garçons</th><th>Total jeunes</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${FCK.esc(r.name||'—')}</td><td>${FCK.esc(r.locality||'—')}</td><td>${n(r.responsibles)}</td><td>${n(r.associations)}</td><td>${n(r.girls)}</td><td>${n(r.boys)}</td><td><strong>${n(r.girls)+n(r.boys)}</strong></td></tr>`).join('')||'<tr><td colspan="7">Aucun secteur enregistré.</td></tr>'}<tr><td colspan="2"><strong>TOTAL GÉNÉRAL</strong></td><td><strong>${n(totals.responsibles)}</strong></td><td><strong>${n(totals.associations)}</strong></td><td><strong>${n(totals.girls)}</strong></td><td><strong>${n(totals.boys)}</strong></td><td><strong>${totalYoung}</strong></td></tr></tbody></table>`;
