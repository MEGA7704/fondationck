(()=>{
  ...
  function renderUsers(){
    ...
    el.innerHTML=`<table><thead><tr><th>Nom</th><th>E-mail</th><th>Rôle / statut</th><th>État</th><th>Plan</th><th>Expiration</th><th>Actions</th></tr></thead><tbody>${filtered.map(u=>`<tr><td>${FCK.esc(u.full_name)}</td><td>${FCK.esc(u.email)}</td><td><span class="badge badge-blue">${FCK.esc(FCK.roleLabel(u))}</span></td><td><span class="badge ${u.status==='active'?'badge-green':'badge-red'}">${FCK.esc(u.status)}</span></td><td><span class="badge ${u.plan==='free'?'badge-orange':'badge-green'}">${FCK.esc(u.plan)}</span></td><td>${FCK.fmtDate(u.plan_expires_at)}</td><td>${u.role==='superadmin'?'<span class="hint">Compte protégé</span>':`<div class="actions"><button class="btn btn-sm btn-orange" data-type="${u.id}">Rôle</button><button class="btn btn-sm btn-ghost" data-status="${u.id}">${u.status==='active'?'Désactiver':'Activer'}</button><button class="btn btn-sm btn-outline" data-plan="${u.id}">Plan</button><button class="btn btn-sm btn-outline" data-reset="${u.id}">Réinit. MDP</button>${u.access?.account_type==='agent'?`<button class="btn btn-sm btn-ghost" data-access="${u.id}">Accès</button>`:''}<button class="btn btn-sm btn-danger" data-delete="${u.id}">Supprimer</button></div>`}</td></tr>`).join('')||'<tr><td colspan="7">Aucun compte.</td></tr>'}</tbody></table>`;
