(async()=>{
  async function render(){
    try{
      ...
      grid.innerHTML=d.news.map(n=>`<a class="news-card" href="/actualite.html?id=${encodeURIComponent(n.id)}"><div class="news-thumb">${n.image_url?`<img src="${n.image_url}" alt="${FCK.esc(n.title)}">`:'<img src="/assets/logo-fondation-ck.png" alt="Logo LA FONDATION CK" style="object-fit:contain;padding:20px;background:#fff">'}</div><div class="news-body"><div class="news-date">${FCK.fmtDate(n.published_at)}</div><h3>${FCK.esc(n.title)}</h3><p>${FCK.esc(n.summary||'Lire la publication complète')}</p></div></a>`).join('');
