(()=>{
  ...
  function render(){
    ...
    document.getElementById('reportStats').innerHTML=`
      ...
      <div class="stat"><small>Alliés des jeunes filles</small><strong>${n(totals.girl_allies)}</strong></div>
      <div class="stat"><small>Jeunes garçons</small><strong>${n(totals.boys)}</strong></div>`;
    const rows=data.report?.by_sector||[];
    document.getElementById('reportTable').innerHTML=`<table id="reportPrintable"><thead><tr><th>Secteur</th><th>Localité</th><th>Resp. secteur</th><th>Resp. jeunes filles</th><th>Resp. jeunes garçons</th><th>Associations</th><th>Jeunes filles</th><th>Alliés des jeunes filles</th><th>Jeunes garçons</th><th>Total jeunes</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${FCK.esc(r.name||'—')}</td><td>${FCK.esc(r.locality||'—')}</td><td>${FCK.esc(r.sector_responsible_name||'—')}</td><td>${FCK.esc(r.girls_responsible_name||'—')}</td><td>${FCK.esc(r.boys_responsible_name||'—')}</td><td>${n(r.associations)}</td><td>${n(r.girls)}</td><td>${n(r.girl_allies)}</td><td>${n(r.boys)}</td><td><strong>${n(r.girls)+n(r.boys)}</strong></td></tr>`).join('')||'<tr><td colspan="10">Aucun secteur enregistré.</td></tr>'}<tr><td colspan="5"><strong>TOTAL GÉNÉRAL</strong></td><td><strong>${n(totals.associations)}</strong></td><td><strong>${n(totals.girls)}</strong></td><td><strong>${n(totals.girl_allies)}</strong></td><td><strong>${n(totals.boys)}</strong></td><td><strong>${totalYoung}</strong></td></tr></tbody></table>`;
