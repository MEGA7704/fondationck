(()=>{
  ...
  function renderAssociations(){
    ...
    document.getElementById('associationTable').innerHTML=`<table><thead><tr><th>Association</th><th>Responsable</th><th>Domaine / objet</th><th>Création / enregistrement</th><th>Siège / implantation</th><th>Contact</th><th>Statut / description</th><th>Actions</th></tr></thead><tbody>${rows.map(a=>`<tr class="${a.id===selectedId?'row-selected':''}"><td><strong>${esc(a.name)}</strong>${a.acronym?`<br><span class="table-meta">Sigle : ${esc(a.acronym)}</span>`:''}</td><td><strong>${esc(a.responsible_name||'—')}</strong></td><td>${esc(a.activity_area||'—')}</td><td>${a.creation_date?`<strong>${esc(FCK.fmtDate(a.creation_date))}</strong>`:'—'}${a.registration_number?`<br><span class="table-meta">N° ${esc(a.registration_number)}</span>`:''}</td><td>${a.headquarters?`<strong>${esc(a.headquarters)}</strong><br>`:''}<span class="table-meta">${[a.sector_name,a.locality,a.village].filter(Boolean).map(esc).join(' · ')||'—'}</span></td><td>${a.phone?`<strong>${esc(a.phone)}</strong>`:'—'}${a.email?`<br><span class="table-meta">${esc(a.email)}</span>`:''}</td><td>${a.status_label?`<span class="badge badge-green">${esc(a.status_label)}</span>`:''}${a.description?`<br><span class="table-meta">${esc(String(a.description).slice(0,110))}${String(a.description).length>110?'…':''}</span>`:''}</td><td><div class="actions"><button class="btn btn-sm btn-primary" data-select="${a.id}">Membres</button>${!isVisitor()?`<button class="btn btn-sm btn-ghost" data-edit="${a.id}">Modifier</button><button class="btn btn-sm btn-danger" data-delete="${a.id}">Supprimer</button>`:''}</div></td></tr>`).join('')||'<tr><td colspan="8">Aucune association enregistrée.</td></tr>'}</tbody></table>`;
  ...
  async function remove(action,id,label){
    if(isAdmin()){
      if(!await FCK.confirmPopup({title:`Supprimer ${label}`,message:`Voulez-vous vraiment supprimer cet élément (${label}) ?`,detail:'Cette action est définitive et ne peut pas être annulée.'}))return;
