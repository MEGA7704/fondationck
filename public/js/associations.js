(()=>{
  ...
  function render(){
    ...
    document.getElementById('associationPageActions').innerHTML=`${canPrint()?'<button id="printAssociations" class="btn btn-outline">🖨 Imprimer</button>':''}${canAdd()?'<button id="addAssociation" class="btn btn-orange">＋ Ajouter association</button>':''}`;
    document.getElementById('addAssociation')?.addEventListener('click',()=>openAssociationForm());
    document.getElementById('printAssociations')?.addEventListener('click',()=>printSection('associationTable','Liste des associations'));
    renderAssociations();renderDetail();
  }

  function renderAssociations(){
    const q=clean(document.getElementById('associationSearch')?.value).toLowerCase();
    const rows=associationRows().filter(a=>!q||[a.name,a.acronym,a.activity_area,a.locality,a.village,a.phone,a.status_label].some(v=>String(v||'').toLowerCase().includes(q)));
    document.getElementById('associationCount').textContent=`${rows.length} association${rows.length>1?'s':''}`;
    document.getElementById('associationTable').innerHTML=`<table><thead><tr><th>Association</th><th>Domaine</th><th>Secteur</th><th>Localité</th><th>Village</th><th>Contact</th><th>Membres</th><th>Responsables</th><th>Statut</th><th>Actions</th></tr></thead><tbody>${rows.map(a=>`<tr class="${a.id===selectedId?'row-selected':''}"><td><strong>${esc(a.name)}</strong>${a.acronym?`<br><span class="hint">${esc(a.acronym)}</span>`:''}</td><td>${esc(a.activity_area||'—')}</td><td>${esc(a.sector_name||'—')}</td><td>${esc(a.locality||'—')}</td><td>${esc(a.village||'—')}</td><td>${esc(a.phone||a.email||'—')}</td><td>${Number(a.members_count||0)}</td><td>${Number(a.responsibles_count||0)}</td><td><span class="badge badge-green">${esc(a.status_label||'Active')}</span></td><td><div class="actions"><button class="btn btn-sm btn-primary" data-select="${a.id}">Listes</button>${!isVisitor()?`<button class="btn btn-sm btn-ghost" data-edit="${a.id}">Modifier</button><button class="btn btn-sm btn-danger" data-delete="${a.id}">Supprimer</button>`:''}</div></td></tr>`).join('')||'<tr><td colspan="10">Aucune association enregistrée.</td></tr>'}</tbody></table>`;
  ...
  function renderDetail(){
    ...
    document.getElementById('selectedAssociationMeta').textContent=[a.acronym,a.activity_area,a.locality,a.village].filter(Boolean).join(' · ');
    document.getElementById('memberActions').innerHTML=`${canPrint()?'<button id="printMembers" class="btn btn-sm btn-outline">🖨 Imprimer</button>':''}${canAdd()?'<button id="addMember" class="btn btn-sm btn-orange">＋ Membre</button>':''}`;
    document.getElementById('responsibleActions').innerHTML=`${canPrint()?'<button id="printAssocResp" class="btn btn-sm btn-outline">🖨 Imprimer</button>':''}${canAdd()?'<button id="addAssocResp" class="btn btn-sm btn-orange">＋ Responsable</button>':''}`;
    document.getElementById('addMember')?.addEventListener('click',()=>openMemberForm({association_id:a.id}));
    document.getElementById('addAssocResp')?.addEventListener('click',()=>openResponsibleForm({association_id:a.id}));
    document.getElementById('printMembers')?.addEventListener('click',()=>printSection('memberTable',`Membres — ${a.name}`));
    document.getElementById('printAssocResp')?.addEventListener('click',()=>printSection('associationResponsibleTable',`Responsables — ${a.name}`));
    renderMembers();renderAssociationResponsibles();
  }

  function renderMembers(){
    const rows=(data.association_members||[]).filter(m=>m.association_id===selectedId);
    document.getElementById('memberTable').innerHTML=`<table><thead><tr><th>Nom</th><th>Sexe</th><th>Contact</th><th>Localité</th><th>Village</th><th>Activité</th><th>Adhésion</th><th>Statut</th><th>Actions</th></tr></thead><tbody>${rows.map(m=>`<tr><td>${esc(m.full_name)}</td><td>${esc(m.gender||'—')}</td><td>${esc(m.phone||m.email||'—')}</td><td>${esc(m.locality||'—')}</td><td>${esc(m.village||'—')}</td><td>${esc(m.occupation||'—')}</td><td>${FCK.fmtDate(m.joined_at)}</td><td>${esc(m.status_label||'Actif')}</td><td>${isVisitor()?'—':`<div class="actions"><button class="btn btn-sm btn-ghost" data-edit-member="${m.id}">Modifier</button><button class="btn btn-sm btn-danger" data-delete-member="${m.id}">Supprimer</button></div>`}</td></tr>`).join('')||'<tr><td colspan="9">Aucun membre enregistré.</td></tr>'}</tbody></table>`;
  ...
  function openAssociationForm(row={}){
    ...
    FCK.modal({title:`${editing?'Modifier':'Ajouter'} une association`,large:true,html:`<form id="associationForm" class="form-grid"><div class="field"><label>Nom de l’association *</label><input class="input" name="name" value="${esc(row.name||'')}" required></div><div class="field"><label>Sigle / acronyme</label><input class="input" name="acronym" value="${esc(row.acronym||'')}"></div><div class="field"><label>Domaine / objet</label><input class="input" name="activity_area" value="${esc(row.activity_area||'')}"></div><div class="field"><label>Date de création</label><input class="input" type="date" name="creation_date" value="${esc(row.creation_date||'')}"></div><div class="field"><label>N° récépissé / enregistrement</label><input class="input" name="registration_number" value="${esc(row.registration_number||'')}"></div><div class="field"><label>Siège</label><input class="input" name="headquarters" value="${esc(row.headquarters||'')}"></div><div class="field"><label>Secteur d’implantation</label><select class="select" id="assocSector" name="sector_id">${sectorOptions(row.sector_id||'')}</select></div><div class="field"><label>Localité</label><input class="input" id="assocLocality" name="locality" value="${esc(row.locality||'')}"></div><div class="field"><label>Village</label><input class="input" id="assocVillage" name="village" value="${esc(row.village||'')}"></div><div class="field"><label>Téléphone</label><input class="input" name="phone" value="${esc(row.phone||'')}"></div><div class="field"><label>E-mail</label><input class="input" type="email" name="email" value="${esc(row.email||'')}"></div><div class="field"><label>Statut</label><input class="input" name="status_label" value="${esc(row.status_label||'Active')}"></div><div class="field full"><label>Description / objectifs</label><textarea class="textarea" name="description">${esc(row.description||'')}</textarea></div>${approvalField(editing)}<div class="field full"><button class="btn btn-primary" type="submit">Enregistrer</button></div></form>`,onReady:(wrap,close)=>{
      ...
      wrap.querySelector('#associationForm').addEventListener('submit',async e=>{e.preventDefault();const obj=Object.fromEntries(new FormData(e.currentTarget));if(editing)obj.id=row.id;try{await FCK.save(editing?'update-association':'add-association',obj);FCK.toast('Association enregistrée.');close();await refresh();if(!selectedId)selectedId=row.id||data.associations?.[0]?.id||'';render()}catch(err){FCK.toast(err.message,'error')}})
    }})
  }
  ...
  function openResponsibleForm(row={}){
    const editing=!!row.id,a=selected();
    FCK.modal({title:`${editing?'Modifier':'Ajouter'} un responsable`,html:`<form id="assocRespForm" class="form-grid"><div class="field full"><label>Association *</label><select class="select" name="association_id" required>${associationOptions(row.association_id||a?.id||'')}</select></div><div class="field"><label>Nom complet *</label><input class="input" name="full_name" value="${esc(row.full_name||'')}" required></div><div class="field"><label>Fonction *</label><input class="input" name="function_title" value="${esc(row.function_title||'Responsable')}" required></div><div class="field"><label>Téléphone</label><input class="input" name="phone" value="${esc(row.phone||'')}"></div><div class="field"><label>E-mail</label><input class="input" type="email" name="email" value="${esc(row.email||'')}"></div><div class="field"><label>Localité</label><input class="input" name="locality" value="${esc(row.locality||a?.locality||'')}"></div><div class="field"><label>Village</label><input class="input" name="village" value="${esc(row.village||a?.village||'')}"></div>${approvalField(editing)}<div class="field full"><button class="btn btn-primary" type="submit">Enregistrer</button></div></form>`,onReady:(wrap,close)=>wrap.querySelector('#assocRespForm').addEventListener('submit',async e=>{e.preventDefault();const obj=Object.fromEntries(new FormData(e.currentTarget));if(editing)obj.id=row.id;try{await FCK.save(editing?'update-association-responsible':'add-association-responsible',obj);selectedId=obj.association_id;FCK.toast('Responsable enregistré.');close();await refresh();render()}catch(err){FCK.toast(err.message,'error')}})})
  }
  ...
  function printSection(id,title){
    ...
    w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${esc(title)}</title><style>body{font-family:Arial,sans-serif;padding:24px;color:#16241f}h1{font-size:20px}table{width:100%;border-collapse:collapse;font-size:12px}th,td{border:1px solid #bbb;padding:7px;text-align:left}.actions,button{display:none}</style></head><body><h1>LA FONDATION CK — ${esc(title)}</h1>${node.innerHTML}</body></html>`);w.document.close();w.focus();setTimeout(()=>w.print(),150)
  }
